...
def scaling(x, y, sigma=0.03):
    factor = np.random.normal(loc=1., scale=sigma, size=(x.shape[0],y))
    return np.multiply(x, factor[:,np.newaxis,:])

def rotation(x):
    flip = np.random.choice([-1, 1], size=(x.shape[0],y))
    rotate_axis = np.arange(y)
...
def permutation(x, max_segments=5, seg_mode="equal"):
    ...
    num_segs = np.random.randint( max_segments, size=(x.shape[0]))
...
def magnitude_warp(x, y, sigma=0.2, knot=4):
    ...
    random_warps = np.random.normal(loc=1.0, scale=sigma, size=(x.shape[0], knot+2, y))
    warp_steps = (np.ones((y,1))*(np.linspace(0, x.shape[1]-1., num=knot+2))).T
    ret = np.zeros_like(x)
    for i, pat in enumerate(x):
        warper = np.array([CubicSpline(warp_steps[:,dim], random_warps[i,:,dim])(orig_steps) for dim in range(y)]).T
        ret[i] = pat * warper
...
def spawner(x, labels, y, sigma=0.05, verbose=0):
